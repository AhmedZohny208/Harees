...
import { registerArea, clearErrors } from 'redux/actions/Areas'
import { queryTeams } from 'redux/actions/Teams'
import { APP_PREFIX_PATH } from 'configs/AppConfig';
import { CREATE_AREA_RESET } from 'redux/constants/Areas';
...
export default function Form() {
  ...
  const { area, error, loading } = useSelector(state => state.registerArea);
  ...
  useEffect(() => {
    ...
    if (area) {
      ...
      dispatch({ type: CREATE_AREA_RESET })
    }
    if (error) {
			setAlertError(error)
  ...
  const handleSubmit = (e) => {
    ...
    if (formValues.title !== '') {
      dispatch(registerArea(formValues))
    }
  }
